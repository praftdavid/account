...
export async function renderSecuritiesFx(container) {
  ...
  const { data: txns, error: txnErr } = finAccountIds.length
    ? await supabase.from('raw_transactions').select('*').in('fin_account_id', finAccountIds).order('txn_date', { ascending: false })
    : { data: [], error: null };
  if (txnErr) {
  ...
  <div class="card">
    ...
    <div style="overflow-x:auto"><table>
      ...
      ${rows || '<tr><td colspan="6" class="note">환전 내역이 없습니다.</td></tr>'}
    </table></div>
  </div>`;
}
